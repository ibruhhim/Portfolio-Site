...
const AwardCard: React.FC<AwardCardProps> = ({
  ...
  const getHoverGlow = () => {
    switch (medalType) {
      ...
      case 'bronze':
        return 'hover:shadow-[0_0_30px_rgba(251,191,36,0.5)]';
      default:
        return 'hover:shadow-[0_0_30px_rgba(252,211,77,0.5)]';
    }
  };
  ...
  return (
    <div className={`bg-emerald-950 border-2 ${getBorderColor()} flex flex-col gap-6 text-white shadow-lg rounded-2xl p-8 hover:shadow-2xl transition-all duration-300 w-full sm:max-w-sm hover:-translate-y-2 hover:border-amber-300/40 ${getHoverGlow()}`}>
      {/* Icon with gradient background */}
      <div className="flex justify-center">
        <div className={`relative w-20 h-20 rounded-full bg-linear-to-br ${getGradient()} flex items-center justify-center shadow-xl transform transition-transform duration-300 hover:scale-110 hover:rotate-6 ring-2 ring-amber-300/30`}>
